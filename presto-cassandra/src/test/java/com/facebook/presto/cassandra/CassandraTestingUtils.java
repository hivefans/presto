...
public class CassandraTestingUtils
{
    ...
    public static void createTableAllTypes(CassandraSession session, SchemaTableName table, Date date, Integer rowsCount)
    {
        session.execute("DROP TABLE IF EXISTS " + table);
        session.execute("CREATE TABLE " + table + " (" +
    ...
    private static void insertTestData(CassandraSession session, SchemaTableName table, Date date, Integer rowsCount)
    {
        for (Integer rowNumber = 1; rowNumber <= rowsCount; rowNumber++) {
            Insert insert = QueryBuilder.insertInto(table.getSchemaName(), table.getTableName())
        ...
        assertEquals(session.execute("SELECT COUNT(*) FROM " + table).all().get(0).getLong(0), rowsCount.longValue());
    }
}
